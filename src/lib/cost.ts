...
export const estimateTotalCost = (textTokens: number): number => {
...
export const checkBudget = (
  inputText: string
): { withinBudget: boolean; estimatedCost: number } => {
  const inputTokens = estimateTokens(inputText);
  const estimatedCost = estimateTotalCost(inputTokens);

  // Budget limit: £0.10 per generation
  const withinBudget = estimatedCost < 0.1;
...
export const summarizeText = (text: string, maxChars: number = 800): string => {
  ...
  if (remainingChars > 20) {
    ...
    return `${firstSentence}. Keywords: ${truncateText(
      keyWords,
      remainingChars
    )}`;
  }
